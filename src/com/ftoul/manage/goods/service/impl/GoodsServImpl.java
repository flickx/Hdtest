...
import com.ftoul.manage.goods.service.GoodsServ;
...
public class GoodsServImpl implements GoodsServ {
	...
	public Result saveGoods(Parameter param) throws Exception {
		...
		if ("0".equals(goods.getCrossborder())) {
			...
			CrossBorderMuseum c = (CrossBorderMuseum)hibernateUtil.find(CrossBorderMuseum.class, goods.getCrossBorderMuseum().getId());
			goods.setCrossBorderMuseum(c);
		}
		//更新商品主图
...
//				+ "where gs.id = gp.goods.id  and gs.state='1' and gp.state = '1' and gs.goodsType3.id = gt.id and gs.goodsBrand.id = gb.id and gp.stock<="+param.getId()+  " group by gs.id";
	...
	public Result getGoodsTypeSetByGoodsId(Parameter param)
			throws Exception {
		Goods goods = (Goods) this.hibernateUtil.find(Goods.class, param.getId()+"");
		GoodsTypeSetVo goodsTypeSetVo = new GoodsTypeSetVo();
		GoodsType goodsType =goods.getGoodsType3();
		//如果是3级分类商品
		if(goodsType.getLevel().equals("3")){
			goodsTypeSetVo.setGoodsTypeLevel3(goodsType);
			String hql="from GoodsType where state=1 and level=2 and id= '"+goodsType.getPid()+"'";
			GoodsType goodsTypeLevel2 = (GoodsType) this.hibernateUtil.hqlFirst(hql);
			goodsTypeSetVo.setGoodsTypeLevel2(goodsTypeLevel2);
			String hql1 ="from GoodsType where state=1 and level=1 and id= '"+goodsTypeLevel2.getPid()+"'";
			GoodsType goodsTypeLevel1 = (GoodsType) this.hibernateUtil.hqlFirst(hql1);
			goodsTypeSetVo.setGoodsTypeLevel1(goodsTypeLevel1);
		}
		else if(goodsType.getLevel().equals("2")){
			goodsTypeSetVo.setGoodsTypeLevel2(goodsType);
			String hql1 ="from GoodsType where state=1 and level=1 and id= '"+goodsType.getPid()+"'";
			GoodsType goodsTypeLevel1 = (GoodsType) this.hibernateUtil.hqlFirst(hql1);
			goodsTypeSetVo.setGoodsTypeLevel1(goodsTypeLevel1);
		}
		return ObjectToResult.getResult(goodsTypeSetVo);
	}
